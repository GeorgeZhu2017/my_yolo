...
class DarkNet(nn.Module):
    ...
    def _make_layer(self, channels, num_blocks):
        layers = []
        layers.append(("ds_conv", nn.Conv2d(self.in_channels, channels[1], kernel_size=3, stride=2, padding=1, bias=False)))
        layers.append(("ds_bn", nn.BatchNorm2d(channels[1])))
        layers.append(("ds_relu", nn.LeakyReLU(0.1)))

        self.in_channels = channels[1]
        for i in range(num_blocks):
            layers.append(("residual_{}".format(i), res_block(self.in_channels, channels)))
